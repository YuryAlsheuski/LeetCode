...
public class TypeConcretizer extends JavaCodeModifier {

  private final MultiKeyMap<String, String> rowNumberAndNameToType;

  public TypeConcretizer(String code, MultiKeyMap<String, String> rowNumberAndNameToType) {
    super(code);
  ...
  private ParameterizedType createParameterizedType(AST ast, String type) {
    ...
    for (String arg : typeArguments) {
      if (arg.contains("<")) {
        ...
        var argType =
            "?".equals(arg) ? ast.newWildcardType() : ast.newSimpleType(ast.newName(arg.trim()));
